#项目概述
Local Vision 是一个为纯文本大语言模型提供本地视觉能力的 AI Agent Skill。许多优秀推理模型（如 DeepSeek、Qwen-Coder、CodeLlama 等）不支持图像输入，面对截图、UI 界面、报错弹窗时无法理解。本项目通过在本地调用 Ollama 多模态视觉模型，将图片内容转换为文字描述，让主模型"看得见"。
...
# 复制到 skill 目录（opencode）
~/.config/opencode/skills/local-vision/

# 使用
python vision.py                    # 识别剪贴板
